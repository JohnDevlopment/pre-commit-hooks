# Do not push commits that begin with "squash!" or "fixup!"
...
def main () -> int:
    ...
    if local_rev == zeroes:
        ...
        cp = bashcmdcapout(f"git rev-list -n 1 -E --grep '^(fixup|squash)!' {rng}")
        rev = cp.stdout.rstrip()
        if rev:
            printf("Found fixup/squash commit in %s, not pushing", local_branch, file=STDERR)
            return 1
